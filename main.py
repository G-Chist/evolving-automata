...
def train_ea(args):
    ...
    if not args['load_data_directly']:
        ...
        pos_data = data.generate_dataset(
            args['pos_reg'], args['n_data'], args['max_repeat'], args['train_split'])
        neg_data = data.generate_dataset(
            args['neg_reg'], args['n_data'], args['max_repeat'], args['train_split'])
    else:
    ...
    population = ea.initialize_pop(
        args['pop_size'], args['min_depth'], args['max_init_depth'], args['width'])
    fitness = ea.evaluate_fitness(
        population, pos_data['train'], neg_data['train'])
    ...
    for i in range(args['ea_epochs']):
        ...
        if (i + 1) % args['print_every'] == 0:
            ...
            best_fitness = ea.evaluate_fa(
                best_individual, pos_data['train']) - ea.evaluate_fa(best_individual, neg_data['train'])
            print("Fitness: %d" % (best_fitness))
            print("Len: %d" % (len(pos_data['train'])))
            print("Best training accuracy: %.1f%%" %
                  (100 * best_fitness / len(pos_data['train'])))
            ea.print_fa(best_individual)
            print()

    best_fitness = ea.evaluate_fa(
        best_individual, pos_data['test']) - ea.evaluate_fa(best_individual, neg_data['test'])
...
if __name__ == '__main__':
    ...
    print("Min accuracy: %.1f \nMax accuracy: %.1f" %
          (min(ea_accuracies), max(ea_accuracies)))
    print("Average accuracy: %.1f \nStandard deviation: %.1f" %
          (np.mean(ea_accuracies), np.std(ea_accuracies)))

    if len(lstm_accuracies) > 0:
        print()
        print("LSTM Results")
        print("Min accuracy: %.1f \nMax accuracy: %.1f" %
              (min(lstm_accuracies), max(lstm_accuracies)))
        print("Average accuracy: %.1f \nStandard deviation: %.1f" %
              (np.mean(lstm_accuracies), np.std(lstm_accuracies)))
